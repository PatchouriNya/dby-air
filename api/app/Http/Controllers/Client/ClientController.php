...
class ClientController extends Controller
{
    ...
    public function getChildrenByAccount($id)
    {
        ...
        $client = new Client();
        $info = Client::with(['withOverview'])->find($client_id);
        $pageSize = request()->query('size'); // 每页显示的孩子数量
        $keyword = request()->query('keyword'); // 要搜索的关键字
        if ($info->type == 1) {
            $data = $info->toArray();
            $data['overview'] = $data['with_overview'];
            $arr = [];
            $arr[] = $data;
            return api($arr, 200, '获取客户信息成功');
        } else {
    ...
    public function expireTime($id, Request $request)
    {
        $expire_time = $request->input('expire_time');
        $expire_time = Carbon::createFromFormat('Y-m-d', $expire_time);
        $grace_time = $request->input('grace_time');
        if ($grace_time) {
            // 解析字符串
            list($quantity, $unit) = $this->parseDurationString($grace_time);
            switch ($unit) {
                case 'd':
                    $expiryDate = $expire_time->addDays($quantity);
                    $grace_time = $quantity . '天';
                    break;
                case 'm':
                    $expiryDate = $expire_time->addMonths($quantity);
                    $grace_time = $quantity . '月';
                    break;
                case 'y':
                    $expiryDate = $expire_time->addYears($quantity);
                    $grace_time = $quantity . '年';
                    break;
                default:
                    return api(null, 400, '宽限时间错误');
            }
        }

        Client::where('id', $id)->update(['expire_time' => $expire_time, 'grace_time' => $grace_time]);


        return api($expire_time, 200, '租期设置成功');
    }

    private function parseDurationString($durationStr)
    {
        // 使用正则表达式解析数量和单位
        preg_match('/^(\d+)([dmy])$/', $durationStr, $matches);
        if (!isset($matches[1]) || !isset($matches[2])) {
            throw new \InvalidArgumentException('Invalid duration string format.');
        }
